...
async function openClient(uid, tab) {
  ...
  { const b = $('#c-aicall'); if (b) b.onclick = () => { if (!u.phone) return alert('У клиента не указан телефон — добавьте через «Изменить».'); openAiCallForm({ type: 'client', id: u.id, name: u.name || u.email, phone: u.phone, onDone: () => openClient(uid, 'aicalls') }); }; }
  $('#c-balance').onclick = () => {
...
async function renderAdmPlans() {
  ...
  const openPlanModal = plan => {
    ...
    // Авторасчёт: базовая цена за тест = пакет без скидки; меняем что-то одно — второе пересчитывается
    const qtyEl = $('#pm-qty'), priceEl = $('#pm-price'), saveEl = $('#pm-save');
    const baseRate = () => {
      const zero = d.plans.filter(x => x.id !== p.id && (x.save || 0) === 0 && x.qty > 0 && x.price > 0).sort((a, b) => a.qty - b.qty)[0];
      if (zero) return zero.price / zero.qty;
      const rates = d.plans.filter(x => x.id !== p.id && x.qty > 0 && x.price > 0).map(x => x.price / x.qty);
      if (rates.length) return Math.max(...rates);
      const q = +qtyEl.value, pr = +priceEl.value; return q && pr ? pr / q : 0;
    };
    const recalcDiscount = () => { const base = baseRate(), q = +qtyEl.value, pr = +priceEl.value;
      if (!base || !q) return; saveEl.value = Math.max(0, Math.min(99, Math.round((1 - (pr / q) / base) * 100))); };
    const recalcPrice = () => { const base = baseRate(), q = +qtyEl.value, dv = +saveEl.value;
      if (!base || !q) return; priceEl.value = Math.max(0, Math.round(q * base * (1 - dv / 100))); };
    qtyEl.oninput = recalcDiscount;
    priceEl.oninput = recalcDiscount;
    saveEl.oninput = recalcPrice;
