...
const I18N = {
  ...
  pl: {
    ...
    tp_none: 'Testy nie zostały jeszcze wysłane.', tst_done: 'Ukończony', tst_prog: 'W trakcie', tst_wait: 'Czekamy na odpowiedź', tp_started: 'Rozpoczął', tp_finished: 'Zakończył', tp_time: 'Czas', tp_status: 'Status', tp_link: 'Link do testu', tp_copy: 'Kopiuj link', tp_see: 'Zobacz wynik →', tp_open: 'Otwórz test', tp_del: 'Usuń test', prev_test: 'Poprzedni test', next_test: 'Następny test', rep_print: 'Drukuj', rep_vacancy: 'Wakat', rep_close: 'Zamknij', ai_hint: 'Podpowiedź AI', synd_h: 'Syndromy i przewagi', z_vlow: 'Bardzo niski', z_low: 'Niski', z_mid: 'Średni', z_high: 'Wysoki', z_vhigh: 'Bardzo wysoki',
...
function showLinksModal(created, lang, vacName) {
  ...
  const groups = Object.keys(byEmail).map(email => {
    ...
    const links = byEmail[email].map(c => `<div class="pt-row"><span class="muted">${esc(testTitle(c.type))}</span><a href="${c.link}" target="_blank" style="max-width:62%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(c.link)}</a></div>`).join('');
    const preview = isSms
  ...
  openModal(`<div class="done-mark" style="color:var(--good);margin:0 auto 4px">${_svg('<circle cx="12" cy="12" r="10"/><path d="M8 12.5l2.5 2.5L16 9"/>')}</div><h2 style="text-align:center;margin-top:2px">${t('links_sent')}</h2>
    <p class="muted" style="text-align:center;margin:6px 0 18px">${t('links_sub')}</p>
