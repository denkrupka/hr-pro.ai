...
async function validate(env, field, answer, ctx) {
  ...
  const system = [
    'Ты — строгий, но доброжелательный наставник по методике «продукт должности» (типология Виннер/Дуэр/Вейтер).',
...
async function assemble(env, ctx) {
  ...
  const system = [
    'Ты собираешь «продукт должности» в одну строку по методике Виннер/Дуэр.',
