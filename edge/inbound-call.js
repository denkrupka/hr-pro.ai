...
function buildStatusBlock(part, vac, owner, lang) {
  ...
  // Недозвон «первого контакта»: кандидат, скорее всего, перезванивает на пропущенный вызов —
  // ИИ должен поблагодарить за перезвон, объяснить, что звонила по отклику, и провести первый контакт.
  const firstTried = log.some(e => e.kind === 'first');
  const anyAnswered = log.some(e => (e.attempts || []).some(a => String(a.transcript || '').trim().length >= 20));
  if (firstTried && !anyAnswered) {
    lines.push(`ВАЖНО — ПРОПУЩЕННЫЙ ПЕРВЫЙ ЗВОНОК: мы звонили этому кандидату ПЕРВЫЙ раз по его отклику на вакансию «${position || '—'}»${company ? ' в компанию ' + company : ''}, но не дозвонились — сейчас он, скорее всего, перезванивает на пропущенный вызов. Обязательно: поблагодари, что перезвонил; сразу объясни, что ты звонила по его отклику на эту вакансию; и проведи короткий ПЕРВЫЙ КОНТАКТ — пара слов о вакансии, уточни интерес, расскажи о следующем шаге процесса (первый тест придёт на почту/в SMS), ответь на вопросы.`);
  }
...
export async function buildInboundAssistant(env, S, caller) {
  ...
  sys += '\nЧТО ДЕЛАТЬ:\n'
    ...
    + '- ОРИЕНТИРУЙСЯ на РЕАЛЬНЫЕ вакансии кандидата (перечислены выше) — используй ИХ точные названия. Если кандидат называет должность, которой НЕТ среди его вакансий, не подменяй и не выдумывай: мягко уточни, о какой из его вакансий идёт речь.\n'
