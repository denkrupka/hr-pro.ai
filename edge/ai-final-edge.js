// Итоговый ИИ-анализ кандидата (edge/Workers-версия src/ai-final.js). Claude через env.ANTHROPIC_API_KEY.
...
export async function finalAssessment(env, ctx) {
...
Дай итоговое заключение по форме.`;
  const out = await callClaude(env, system, user, 3200);
  const jj = parseJson(out) || {};
  const verdict = ['hire', 'consider', 'reject'].includes(jj.verdict) ? jj.verdict : 'consider';
  return {
    verdict,
    fit_score: Math.max(0, Math.min(100, parseInt(jj.fit_score, 10) || 0)),
    headline: String(jj.headline || '').slice(0, 300),
    summary: String(jj.summary || '').slice(0, 2000),
    strengths: Array.isArray(jj.strengths) ? jj.strengths.slice(0, 8).map(String) : [],
    concerns: Array.isArray(jj.concerns) ? jj.concerns.slice(0, 8).map(String) : [],
    recommendation: String(jj.recommendation || '').slice(0, 800),
  };
}
