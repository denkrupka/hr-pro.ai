...
function languageRule(language) {
  ...
  return `ЯЗЫК РАЗГОВОРА: веди беседу на ${L} языке — это язык заявки. Если кандидат отвечает на ДРУГОМ языке (например, украинском, польском, английском), НЕ переходи на его язык — вежливо продолжай на ${L}. При этом обязательно отметь в итоге, на каком языке фактически говорил кандидат, если он отличается от ${L}. ИМЕНА, а также НАЗВАНИЯ ДОЛЖНОСТЕЙ и КОМПАНИЙ произноси на языке разговора с естественным ударением: если записаны в другом алфавите/языке (например латиницей или по-польски в русском разговоре) — переводи или произноси корректно (Denys→Денис, «Kierownik robót»→«руководитель работ»), НЕ читай по буквам и не по-английски. Не вставляй иностранные слова (по-русски — «рекрутёр», не «recruiter»). ЗАВЕРШЕНИЕ: как только попрощались С ОБЕИХ СТОРОН — СРАЗУ заверши звонок функцией завершения, не держи линию. `;
}
...
async function startCall(settings, { to, task, firstMessage, language, structuredDataSchema, summaryPrompt, maxDurationMin }) {
  ...
  const vmDetect = { provider: 'vapi', backoffPlan: { maxRetries: 10, startAtSeconds: 2, frequencySeconds: 2.5 } };
